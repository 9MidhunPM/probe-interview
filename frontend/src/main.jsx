...
const poseImages = { confident: 'candidate-confident.png', unsure: 'candidate-nervous.png', vague: 'candidate-vague.png' };
...
function Feedback({ candidate, closing, feedback, approach, transcript, onClose }) {
  const answers = transcript.filter((entry) => entry.speaker !== 'Dr. Probey');
  return <div className="feedback-overlay" role="presentation" onClick={onClose}><section className="feedback-modal session-summary" role="dialog" aria-modal="true" aria-labelledby="feedback-title" onClick={(event) => event.stopPropagation()}><header><p className="kicker">Interview complete</p><h2 id="feedback-title">{candidate.member.name}&apos;s session summary</h2><button className="modal-close" type="button" onClick={onClose}>Close</button><p>{candidate.member.jobRole} · {candidate.member.yearsExperience} years experience · {candidate.member.education}</p></header><div className="feedback-body"><section className="session-facts" aria-label="Session details"><article><strong>{answers.length}</strong><span>answers explored</span></article><article><strong>{candidate.missions.length}</strong><span>missions reviewed</span></article><article><strong>{candidate.signals.missionsFirstTry}</strong><span>first-try missions</span></article><article><strong>{candidate.signals.commitDays}</strong><span>active learning days</span></article></section><section className="closing-note"><h3>Dr. Probey&apos;s closing note</h3><p>{closing}</p></section><p className="feedback-summary">{feedback.summary}</p>{approach.length > 0 && <section className="probey-approach"><h3>Dr. Probey&apos;s approach</h3><ul>{approach.map((item) => <li key={item}>{item}</li>)}</ul></section>}<div className="feedback-columns">{[['Strengths', feedback.strengths], ['Gaps', feedback.gaps], ['Next steps', feedback.next]].map(([label, items]) => <section className="summary-column" key={label}><h3>{label}</h3><ul>{items.map((item) => <li key={item}>{item}</li>)}</ul></section>)}</div><section className="session-record"><h3>Conversation record</h3><div>{transcript.map((entry, index) => <article key={`${entry.speaker}-${index}`}><strong>{entry.speaker}</strong><p>{entry.message}</p></article>)}</div></section></div></section></div>;
}

function useWordStream(value, delay) {
  ...
  useEffect(() => {
    ...
    let index = 0;
    setText('');
    if (!chunks.length) { setIsStreaming(false); return undefined; }
    setIsStreaming(true);
    function next() {
      const chunk = chunks[index];
      setText((current) => current + chunk);
      index += 1;
      if (index === chunks.length) setIsStreaming(false);
      else timeout = window.setTimeout(next, delay);
    }
    timeout = window.setTimeout(next, 0);
...
function Trace({ trace, traceHistory, transcript, busy, generating }) {
  const [open, setOpen] = useState(null);
  const workingAgent = generating ? 'Answer Generator' : busy ? 'Interviewer' : null;
  return <aside className="orchestration" data-tutorial="trail"><header><p className="kicker">Live graph</p><h2>Reasoning trail</h2><span>{trace.length} agents this turn</span></header><div className="agent-rail">{agentNames.map((agent) => { const current = trace.find((item) => item.agent === agent); const past = [...traceHistory].reverse().find((item) => item.agent === agent); const entry = current || past; const status = agent === workingAgent ? 'working' : current ? 'complete' : entry ? 'available' : 'idle'; return <section className={`agent-row ${status}`} key={agent}><button type="button" disabled={!entry} onClick={() => setOpen(open === agent ? null : agent)}><span className="agent-dot" /><strong>{agent === 'Interviewer' ? 'Dr. Probey' : agent}</strong><small>{status === 'working' ? 'working' : current ? 'complete' : entry ? 'past' : 'idle'}</small></button>{open === agent && entry && <pre className="trace-inline">{JSON.stringify(entry.output, null, 2)}</pre>}</section>; })}</div><Transcript transcript={transcript} /></aside>;
}
...
function InterviewStage({ candidate, response, transcript, trace, traceHistory, busy, generating, onGenerate, onSend, onEnd, onCloseFeedback }) {
  const [draft, setDraft] = useState('');
  const [feedbackOpen, setFeedbackOpen] = useState(false);
  const [tutorialOpen, setTutorialOpen] = useState(false);
  const [pose, setPose] = useState('idle');
  const [answerToStream, setAnswerToStream] = useState('');
  const [streamedReply, isReplyStreaming] = useWordStream(response.reply, 90);
  const [streamedAnswer, isAnswerStreaming] = useWordStream(answerToStream, 75);
  const approach = response.trace?.find((entry) => entry.agent === 'Evaluator')?.output?.approach || [];
  const replyFinished = streamedReply === response.reply && !isReplyStreaming;
  useEffect(() => { if (answerToStream) setDraft(streamedAnswer); }, [answerToStream, streamedAnswer]);
  useEffect(() => { if (response.done && response.feedback && replyFinished) setFeedbackOpen(true); }, [response, replyFinished]);
  useEffect(() => { if (window.localStorage.getItem('probe_tutorial_seen') !== 'true') setTutorialOpen(true); }, []);
  async function generate(style) { setPose(style); setAnswerToStream(''); const answer = await onGenerate(style); if (answer) setAnswerToStream(answer); }
  async function submit(event) { event.preventDefault(); if (!draft.trim()) return; const answer = draft.trim(); setDraft(''); setAnswerToStream(''); setPose('idle'); await onSend(answer); }
  function closeTutorial() { window.localStorage.setItem('probe_tutorial_seen', 'true'); setTutorialOpen(false); }
  const interviewerThinking = busy && !generating && !isReplyStreaming;
  const interviewerActive = interviewerThinking || isReplyStreaming;
  const candidateImage = generating ? 'candidate-thinking.png' : isAnswerStreaming ? 'candidate-speaking.png' : poseImages[pose] || (draft.trim() ? 'candidate-speaking.png' : 'candidate-idle.png');
  const interviewerImage = isReplyStreaming ? 'interviewer-speaking.png' : interviewerThinking ? 'interviewer-thinking.png' : 'interviewer-idle.png';
  const controlsDisabled = busy || response.done || isReplyStreaming || isAnswerStreaming;
  const questionIntent = trace.find((entry) => entry.agent === 'Interviewer')?.output?.intent || 'Prepare a focused question from the available evidence.';
  return <main className="scene-app"><header className="app-header"><a href="/classic">Classic</a><span>Probe / live practice</span><div className="app-header-actions"><strong>{candidate.member.name}</strong><button type="button" className="replay-tutorial" onClick={() => setTutorialOpen(true)}>Replay tutorial</button></div></header><div className="app-shell"><section className="scene-pane"><section className="interviewer-panel"><div className="turn-speech" data-tutorial="speech" aria-live="polite"><Markdown>{streamedReply || '...'}</Markdown></div><div className="question-intent" data-tutorial="intent"><strong>Question intent</strong><span>{questionIntent}</span></div></section><section className="scene-frame" aria-label="Interview room"><div className="scene-camera"><img className="scene-backdrop" src={asset('interview-room.png')} alt="" /><div className={`speaker interviewer-speaker ${interviewerActive ? 'is-active' : 'is-idle'}`}><img className="scene-character interviewer-character" src={asset(interviewerImage)} alt="Dr. Probey" /></div><div className={`speaker candidate-speaker ${interviewerActive ? 'is-idle' : 'is-active'}`}><img className="scene-character candidate-character" src={asset(candidateImage)} alt={candidate.member.name} /></div></div></section><form className="candidate-panel" onSubmit={submit}><p className="turn-label">{candidate.member.name}</p><section className="composer-slot" data-tutorial="compose"><span>{response.done ? 'Interview complete' : 'Compose your response'}</span><textarea name="candidate-response" value={draft} onChange={(event) => { setDraft(event.target.value); setAnswerToStream(''); if (pose !== 'idle') setPose('idle'); }} placeholder="Write or edit a response" disabled={controlsDisabled} /></section><div className="style-rail" data-tutorial="styles">{responseStyles.map(([style, label]) => <button type="button" key={style} onClick={() => generate(style)} disabled={controlsDisabled}>{generating === style ? 'Generating...' : label}</button>)}</div><div className="action-stack"><button className="primary-action" data-tutorial="send" disabled={controlsDisabled || !draft.trim()}>{response.done ? 'Interview complete' : isReplyStreaming ? 'Dr. Probey is speaking...' : busy ? 'Dr. Probey is thinking...' : 'Send answer'}</button><button className="end-interview" type="button" onClick={onEnd} disabled={controlsDisabled}>End interview</button></div></form></section><Trace trace={trace} traceHistory={traceHistory} transcript={transcript} busy={busy} generating={generating} /></div>{tutorialOpen && <Tutorial onClose={closeTutorial} />}{feedbackOpen && <Feedback candidate={candidate} closing={response.reply} feedback={response.feedback} approach={approach} transcript={transcript} onClose={() => { setFeedbackOpen(false); onCloseFeedback(); }} />}</main>;
}
...
function App() {
  ...
  const [error, setError] = useState('');
  async function start(selected) { const id = crypto.randomUUID(); setBusy(true); try { const next = await request('/api/interview', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ sessionId: id, candidate: selected }) }); setCandidate(selected); setSessionId(id); setResponse(next); setTranscript([{ speaker: 'Dr. Probey', message: next.reply }]); setTrace(next.trace || []); setTraceHistory(next.trace || []); } catch (err) { setError(err.message); } finally { setBusy(false); } }
  ...
  async function send(message) { setBusy(true); setTranscript((current) => [...current, { speaker: candidate.member.name, message }]); try { const next = await request('/api/interview', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ sessionId, message }) }); setResponse(next); setTrace(next.trace || []); setTraceHistory((current) => [...current, ...(next.trace || [])]); setTranscript((current) => [...current, { speaker: 'Dr. Probey', message: next.reply }]); } catch (err) { setError(err.message); } finally { setBusy(false); } }
  async function endInterview() { setBusy(true); try { const next = await request('/api/interview/end', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ sessionId }) }); setResponse(next); setTrace(next.trace || []); setTraceHistory((current) => [...current, ...(next.trace || [])]); setTranscript((current) => [...current, { speaker: 'Dr. Probey', message: next.reply }]); } catch (err) { setError(err.message); } finally { setBusy(false); } }
  ...
  return <><InterviewStage candidate={candidate} response={response} transcript={transcript} trace={trace} traceHistory={traceHistory} busy={busy} generating={generating} onGenerate={generate} onSend={send} onEnd={endInterview} onCloseFeedback={() => {}} />{error && <p className="toast" role="alert">{error}</p>}</>;
}
