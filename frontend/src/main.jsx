...
function Feedback({ candidate, closing, feedback, approach, transcript, onClose }) {
  ...
  return <div className="feedback-overlay" role="presentation" onClick={onClose}><section className="feedback-modal session-summary" role="dialog" aria-modal="true" aria-labelledby="feedback-title" onClick={(event) => event.stopPropagation()}><header><p className="kicker">Interview complete</p><h2 id="feedback-title">{candidate.member.name}&apos;s session summary</h2><button className="modal-close" type="button" onClick={onClose}>Close</button><p>{candidate.member.jobRole} · {candidate.member.yearsExperience} years experience · {candidate.member.education}</p></header><div className="feedback-body"><section className="session-facts" aria-label="Session details"><article><strong>{answers.length}</strong><span>answers explored</span></article><article><strong>{candidate.missions.length}</strong><span>missions reviewed</span></article><article><strong>{candidate.signals.missionsFirstTry}</strong><span>first-try missions</span></article><article><strong>{candidate.signals.commitDays}</strong><span>active learning days</span></article></section><section className="closing-note"><h3>Dr. Probey&apos;s closing note</h3><p>{closing}</p></section><p className="feedback-summary">{feedback.summary}</p>{approach.length > 0 && <section className="probey-approach"><h3>Dr. Probey&apos;s approach</h3><ul>{approach.map((item) => <li key={item}>{item}</li>)}</ul></section>}{[['Strengths', feedback.strengths], ['Gaps', feedback.gaps], ['Next steps', feedback.next]].map(([label, items]) => <section className="summary-column" key={label}><h3>{label}</h3><ul>{items.map((item) => <li key={item}>{item}</li>)}</ul></section>)}<section className="session-record"><h3>Conversation record</h3><div>{transcript.map((entry, index) => <article key={`${entry.speaker}-${index}`}><strong>{entry.speaker}</strong><p>{entry.message}</p></article>)}</div></section></div></section></div>;
}

function useWordStream(value) {
  ...
  useEffect(() => {
    ...
    function next() {
      ...
      else timeout = window.setTimeout(next, 18);
    }
...
function InterviewStage({ candidate, response, transcript, trace, busy, generating, onGenerate, onSend, onEnd, onCloseFeedback }) {
  const [draft, setDraft] = useState('');
  const [feedbackOpen, setFeedbackOpen] = useState(false);
  const [tutorialOpen, setTutorialOpen] = useState(false);
  const [pose, setPose] = useState('idle');
  const [answerToStream, setAnswerToStream] = useState('');
  const [streamedReply, isReplyStreaming] = useWordStream(response.reply);
  const [streamedAnswer, isAnswerStreaming] = useWordStream(answerToStream);
  ...
  return <main className="scene-app"><header className="app-header"><a href="/classic">Classic</a><span>Probe / live practice</span><button type="button" className="replay-tutorial" onClick={() => setTutorialOpen(true)}>Replay tutorial</button><strong>{candidate.member.name}</strong></header><div className="app-shell"><section className="scene-pane"><section className="interviewer-panel"><div className="turn-speech" data-tutorial="speech"><Markdown>{streamedReply || '...'}</Markdown></div></section><section className="scene-frame" aria-label="Interview room"><div className="scene-camera"><img className="scene-backdrop" src={asset('interview-room.png')} alt="" /><div className={`speaker interviewer-speaker ${interviewerActive ? 'is-active' : 'is-idle'}`}><img className="scene-character interviewer-character" src={asset(interviewerImage)} alt="Dr. Probey" /></div><div className="question-intent" data-tutorial="intent"><strong>Question intent</strong><span>Understand the reasoning behind your approach.</span></div><div className={`speaker candidate-speaker ${interviewerActive ? 'is-idle' : 'is-active'}`}><img className="scene-character candidate-character" src={asset(candidateImage)} alt={candidate.member.name} /></div></div></section><form className="candidate-panel" onSubmit={submit}><p className="turn-label">{candidate.member.name}</p><section className="composer-slot" data-tutorial="compose"><span>{response.done ? 'Interview complete' : 'Compose your response'}</span><textarea name="candidate-response" value={draft} onChange={(event) => { setDraft(event.target.value); setAnswerToStream(''); if (pose !== 'idle') setPose('idle'); }} placeholder="Write or edit a response" disabled={controlsDisabled} /></section><div className="style-rail" data-tutorial="styles">{responseStyles.map(([style, label]) => <button type="button" key={style} onClick={() => generate(style)} disabled={controlsDisabled}>{generating === style ? 'Generating...' : label}</button>)}</div><div className="action-stack"><button className="end-interview" type="button" onClick={onEnd} disabled={controlsDisabled}>End interview</button><button className="primary-action" data-tutorial="send" disabled={controlsDisabled || !draft.trim()}>{response.done ? 'Interview complete' : isReplyStreaming ? 'Dr. Probey is speaking...' : busy ? 'Dr. Probey is thinking...' : 'Send answer'}</button></div></form></section><Trace trace={trace} transcript={transcript} /></div>{tutorialOpen && <Tutorial onClose={closeTutorial} />}{feedbackOpen && <Feedback candidate={candidate} closing={response.reply} feedback={response.feedback} approach={approach} transcript={transcript} onClose={() => { setFeedbackOpen(false); onCloseFeedback(); }} />}</main>;
}
...
function App() {
  ...
  return <><InterviewStage candidate={candidate} response={response} transcript={transcript} trace={trace} busy={busy} generating={generating} onGenerate={generate} onSend={send} onEnd={endInterview} onCloseFeedback={() => {}} />{error && <p className="toast" role="alert">{error}</p>}</>;
}
